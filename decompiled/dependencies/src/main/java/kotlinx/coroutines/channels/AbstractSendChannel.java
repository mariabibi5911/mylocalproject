...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    public final Object mo1659trySendJP2dKIU(E element) {
        Object result = offerInternal(element);
        if (result == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1679successJP2dKIU(Unit.INSTANCE);
        }
        if (result == AbstractChannelKt.OFFER_FAILED) {
            Closed closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1678failurePtdJZtk() : ChannelResult.INSTANCE.m1677closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (result instanceof Closed) {
            return ChannelResult.INSTANCE.m1677closedJP2dKIU(helpCloseAndGetSendException((Closed) result));
        }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (it = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            Result.Companion companion = Result.INSTANCE;
            continuation.resumeWith(Result.m168constructorimpl(ResultKt.createFailure(it)));
        } else {
            Result.Companion companion2 = Result.INSTANCE;
            continuation.resumeWith(Result.m168constructorimpl(ResultKt.createFailure(sendException)));
        }
    }
    ...
    */
    ...
    /* JADX WARN: Code restructure failed: missing block: B:24:0x0032, code lost:
        ...
        r1 = r4;
        r4 = r4 - 1;
        r5 = (kotlinx.coroutines.channels.Receive) r3.get(r1);
    ...
    */
    private final void helpClose(Closed<?> closed) {
        Object closedList = InlineList.m1695constructorimpl$default(null, 1, null);
        while (true) {
            LockFreeLinkedListNode prevNode = closed.getPrevNode();
            Receive previous = prevNode instanceof Receive ? (Receive) prevNode : null;
            if (previous == null) {
                break;
            } else if (!previous.mo1703remove()) {
                previous.helpRemove();
            } else {
                closedList = InlineList.m1700plusFjFbRPM(closedList, previous);
            }
        }
        if (closedList != null) {

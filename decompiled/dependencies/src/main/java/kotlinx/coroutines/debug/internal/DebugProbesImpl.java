...
public final class DebugProbesImpl {
    ...
    private final Function1<Boolean, Unit> getDynamicAttach() {
        ...
        if (newInstance != null) {
            m168constructorimpl = Result.m168constructorimpl((Function1) TypeIntrinsics.beforeCheckcastToFunctionOfArity(newInstance, 1));
            if (Result.m174isFailureimpl(m168constructorimpl)) {
                m168constructorimpl = null;
            }
            return (Function1) m168constructorimpl;
        }
        ...
        try {
            ...
            try {
                ...
                for (Object element$iv$iv2 : $this$associateBy$iv) {
                    ...
                    Job job2 = JobKt.getJob(it2.delegate.get$context());
    ...
    private final List<StackTraceElement> enhanceStackTraceWithThreadDumpImpl(String state, Thread thread, List<StackTraceElement> coroutineTrace) {
        ...
        try {
            Result.Companion companion = Result.INSTANCE;
            DebugProbesImpl debugProbesImpl = this;
            m168constructorimpl = Result.m168constructorimpl(thread.getStackTrace());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            m168constructorimpl = Result.m168constructorimpl(ResultKt.createFailure(th));
        }
        ...
        StackTraceElement[] actualTrace = (StackTraceElement[]) m168constructorimpl;
        if (actualTrace == null) {
            return coroutineTrace;
        }
    ...
    private final <T> Continuation<T> createOwner(Continuation<? super T> completion, StackTraceFrame frame) {
        ...
        DebugCoroutineInfo info = new DebugCoroutineInfo(completion.get$context(), frame, sequenceNumber$FU.incrementAndGet(debugProbesImpl$SequenceNumberRefVolatile));

        ...
        */
        V lockedGetOrLoad(K key, int hash, CacheLoader<? super K, V> loader) throws ExecutionException {
            ...
            try {
                ...
                ReferenceEntry<K, V> referenceEntry = table.get(index2);
                ReferenceEntry<K, V> first2 = referenceEntry;
                ReferenceEntry<K, V> e3 = referenceEntry;
                while (true) {
                    if (e3 == null) {
                        e = e3;
                        loadingValueReference = loadingValueReference3;
                        createNewEntry = createNewEntry2;
                        first = first2;
                        index = index2;
                        break;
                    }
                    try {
                        ...
                        try {
                            ...
                            first2 = first2;
                            index2 = index2;
                            createNewEntry2 = createNewEntry;
                            loadingValueReference3 = loadingValueReference;
                        } catch (Throwable th) {
                            th = th;
                            unlock();
                            postWriteCleanup();
                ...
                if (createNewEntry) {
                    LoadingValueReference<K, V> loadingValueReference4 = new LoadingValueReference<>();
                    try {
                        if (e == null) {
                            e2 = newEntry(key, hash, first);
                            e2.setValueReference(loadingValueReference4);
                            table.set(index, e2);
                            loadingValueReference2 = loadingValueReference4;
                        } else {
                            e.setValueReference(loadingValueReference4);
                            loadingValueReference2 = loadingValueReference4;
                            e2 = e;
                        }
                    } catch (Throwable th3) {
                ...
                postWriteCleanup();
                if (!createNewEntry) {
                    return waitForLoadingValue(e2, key, valueReference);
                }
                try {
                    synchronized (e2) {
                        try {
            ...
            if (!keysToLoad.isEmpty()) {
                try {
                    ...
                    for (K key2 : keysToLoad) {
                        ...
                        newLinkedHashMap.put(key2, value);
                    }
                } catch (CacheLoader.UnsupportedLoadingOperationException e) {
                    for (K key3 : keysToLoad) {
                        misses--;
                        newLinkedHashMap.put(key3, get(key3, this.defaultLoader));
                    }
                }
            }
    ...
    */
    public boolean containsValue(@CheckForNull Object value) {
        ...
        while (i < 3) {
            ...
            int i2 = 0;
            while (i2 < length) {
                Segment<K, V> segment = segments[i2];
                int i3 = segment.count;
                AtomicReferenceArray<ReferenceEntry<K, V>> table = segment.table;
                int j = 0;
                while (true) {
                    Segment<K, V>[] segments2 = segments;
                    if (j < table.length()) {
            ...
            long now3 = now2;
            Segment<K, V>[] segments3 = segments;
            if (sum != last) {
                last = sum;
                i++;
                segments = segments3;

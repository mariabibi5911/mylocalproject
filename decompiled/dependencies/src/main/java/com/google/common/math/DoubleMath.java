...
public final class DoubleMath {
    ...
    */
    public static int log2(double x, RoundingMode mode) {
        ...
        switch (AnonymousClass1.$SwitchMap$java$math$RoundingMode[mode.ordinal()]) {
            case 1:
                boolean increment2 = isPowerOfTwo(x);
                MathPreconditions.checkRoundingUnnecessary(increment2);
            case 2:
                increment = false;
                return increment ? exponent + 1 : exponent;
            case 3:
                boolean increment3 = isPowerOfTwo(x);
                increment = !increment3;
                if (increment) {
                }
                break;
            case 4:
                increment = (!isPowerOfTwo(x)) & (exponent < 0);
                if (increment) {
                }
                break;
            case 5:

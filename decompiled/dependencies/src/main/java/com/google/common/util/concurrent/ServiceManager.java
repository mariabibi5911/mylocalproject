    ...
    public static final class ServiceManagerState {
        ...
        void checkHealthy() {
            if (this.states.count(Service.State.RUNNING) != this.numberOfServices) {
                ...
                IllegalStateException exception = new IllegalStateException(new StringBuilder(String.valueOf(valueOf).length() + 79).append("Expected to be healthy after starting. The following services are not running: ").append(valueOf).toString());
                throw exception;
            }
        }
    }
        ...
        public void failed(Service.State from, Throwable failure) {
            ...
            if (state != null) {
                ...
                if (log) {
                    ...
                    logger.log(level, new StringBuilder(String.valueOf(valueOf).length() + 34 + String.valueOf(valueOf2).length()).append("Service ").append(valueOf).append(" has failed in the ").append(valueOf2).append(" state.").toString(), failure);
                }
